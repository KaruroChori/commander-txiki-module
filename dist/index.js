...
class EventEmitter {
  ...
  off(eventName, callback) {
    this.listeners = this.listeners.filter((listener) => {
      listener.name === eventName && listener.callback;
    });
  }
  destroy() {
  ...
  listenerCount(eventName, listener = undefined) {
    return this.listeners.filter((x) => {
      if (x.callback !== undefined)
        return x.callback === listener;
      else {
...
class Help {
  ...
  argumentDescription(argument2) {
    ...
    if (argument2.argChoices) {
      extraInfo.push(`choices: ${argument2.argChoices.map((choice) => JSON.stringify(choice)).join(", ")}`);
    }
    if (argument2.defaultValue !== undefined) {
...
- specify the name in Command constructor or using .name()`);
  ...
  argument(name, description, fn, defaultValue) {
    const argument3 = this.createArgument(name, description);
    if (typeof fn === "function") {
      argument3.default(defaultValue).argParser(fn);
    } else {
      argument3.default(fn);
    }
    this.addArgument(argument3);
    return this;
  }
  arguments(names) {
  ...
  addArgument(argument3) {
    ...
    if (argument3.required && argument3.defaultValue !== undefined && argument3.parseArg === undefined) {
      throw new Error(`a default value for a required argument is never used: '${argument3.name()}'`);
    }
    this.registeredArguments.push(argument3);
    return this;
  }
...
-  already used by option '${matchingOption.flags}'`);
  ...
  addOption(option2) {
    ...
    if (option2.negate) {
      ...
      if (!this._findOption(positiveLongFlag)) {
        this.setOptionValueWithSource(name, option2.defaultValue === undefined ? true : option2.defaultValue, "default");
      }
    } else if (option2.defaultValue !== undefined) {
      this.setOptionValueWithSource(name, option2.defaultValue, "default");
    }
    const handleOptionValue = (val, invalidValueMessage, valueSource) => {
      if (val == null && option2.presetArg !== undefined) {
 ...
 - ${executableDirMessage}`;
  ...
  _processArguments() {
    const myParseArg = (argument3, value, previous) => {
      let parsedValue = value;
      if (value !== null && argument3.parseArg) {
        const invalidValueMessage = `error: command-argument value '${value}' is invalid for argument '${argument3.name()}'.`;
        parsedValue = this._callParseArg(argument3, value, previous, invalidValueMessage);
      }
      return parsedValue;
    };
  ...
  _checkForConflictingLocalOptions() {
    ...
    const optionsWithConflicting = definedNonDefaultOptions.filter((option2) => option2.conflictsWith.length > 0);
    optionsWithConflicting.forEach((option2) => {
      const conflictingAndDefined = definedNonDefaultOptions.find((defined) => option2.conflictsWith.includes(defined.attributeName()));
  ...
  _parseOptionsImplied() {
    ...
    this.options.filter((option2) => option2.implied !== undefined && hasCustomOptionValue(option2.attributeName()) && dualHelper.valueFromOption(this.getOptionValue(option2.attributeName()), option2)).forEach((option2) => {
