import { EventEmitter, childProcess, fs, process } from './adapter.js';
...
Expecting one of '${allowedValues.join("', '")}'`);
  ...
  _exit(exitCode, code, message) {
    if (this._exitCallback) {
      if(this._exitCallback(new CommanderError(exitCode, code, message)))return;
...
-  already used by option '${matchingOption.flags}'`);
  ...
  async _executeSubCommand(subcommand, args) {
    ...
    async function findFile(baseDir, baseName) {
      ...
      if (await fs.existsAsync(localBin)) return localBin;

      // Stop looking if candidate already has an expected extension.
      if (sourceExt.includes(path.extname(baseName))) return undefined;

      // Try all the extensions.
      const foundExt = sourceExt.find(async(ext) =>
        await fs.existsASync(`${localBin}${ext}`),
      );
 ...
 - ${executableDirMessage}`;
  ...
  async _dispatchSubcommand(commandName, operands, unknown) {
    ...
    if (!subCommand) await this.help({ error: true });
  ...
  async _dispatchHelpCommand(subcommandName) {
    ...
    return await this._dispatchSubcommand(
      subcommandName,
      [],
